...
% modal_eddy_vis1 = mean((P_unres(:,sum_i).*D_res(:,sum_i)),1)'./mean((D_res(:,sum_i).^2),1)';
% modal_eddy_vis(2:end) = mean(((modal_amp(:,sum_i)).^2.*P_unres(:,sum_i).*D_res(:,sum_i)),1)'./mean((modal_amp(:,sum_i).*D_res(:,sum_i).^2),1)';
